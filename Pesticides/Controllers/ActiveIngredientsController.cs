using System;
using System.Collections.Generic;
using System.Data;
using System.Data.Entity;
using System.Linq;
using System.Net;
using System.Web;
using System.Web.Mvc;
using Pesticides.Models;

namespace Pesticides.Controllers
{
    [Authorize]
    public class ActiveIngredientsController : Controller
    {
        private PesticidesDBEntities db = new PesticidesDBEntities();

        // GET: ActiveIngredients
        public ActionResult Index()
        {
            return View(db.ActiveIngredients.ToList());
        }

        // GET: ActiveIngredients/Details/5
        public ActionResult Details(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            ActiveIngredient activeIngredient = db.ActiveIngredients.Find(id);
            if (activeIngredient == null)
            {
                return HttpNotFound();
            }
            return View(activeIngredient);
        }

        // GET: ActiveIngredients/Create
        public ActionResult Create()
        {
            return View();
        }

        // POST: ActiveIngredients/Create
        // To protect from overposting attacks, please enable the specific properties you want to bind to, for 
        // more details see https://go.microsoft.com/fwlink/?LinkId=317598.
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Create([Bind(Include = "ActiveIngredientPk,IngredientAr,IngredientEn")] ActiveIngredient activeIngredient)
        {
            if (ModelState.IsValid)
            {
                db.ActiveIngredients.Add(activeIngredient);
                db.SaveChanges();
                return RedirectToAction("Index");
            }

            return View(activeIngredient);
        }

        // GET: ActiveIngredients/Edit/5
        public ActionResult Edit(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            ActiveIngredient activeIngredient = db.ActiveIngredients.Find(id);
            if (activeIngredient == null)
            {
                return HttpNotFound();
            }
            return View(activeIngredient);
        }

        // POST: ActiveIngredients/Edit/5
        // To protect from overposting attacks, please enable the specific properties you want to bind to, for 
        // more details see https://go.microsoft.com/fwlink/?LinkId=317598.
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "ActiveIngredientPk,IngredientAr,IngredientEn")] ActiveIngredient activeIngredient)
        {
            if (ModelState.IsValid)
            {
                db.Entry(activeIngredient).State = EntityState.Modified;
                db.SaveChanges();
                return RedirectToAction("Index");
            }
            return View(activeIngredient);
        }

        // GET: ActiveIngredients/Delete/5
        public ActionResult Delete(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            ActiveIngredient activeIngredient = db.ActiveIngredients.Find(id);
            if (activeIngredient == null)
            {
                return HttpNotFound();
            }
            return View(activeIngredient);
        }

        // POST: ActiveIngredients/Delete/5
        [HttpPost, ActionName("Delete")]
        [ValidateAntiForgeryToken]
        public ActionResult DeleteConfirmed(int id)
        {
            ActiveIngredient activeIngredient = db.ActiveIngredients.Find(id);
            db.ActiveIngredients.Remove(activeIngredient);
            db.SaveChanges();
            return RedirectToAction("Index");
        }

        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                db.Dispose();
            }
            base.Dispose(disposing);
        }

        [AllowAnonymous]
        public ActionResult VisitMOASite()
        {
            return Redirect("http://www.agriculture.gov.lb");
        }
    }
}
